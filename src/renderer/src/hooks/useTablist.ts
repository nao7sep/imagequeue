...
import { indexOfId, nextIndex } from '../utils/compositeNav'

// The app's tablist layer: one shared hook behind every tab bar (currently the
// Settings modal's section tabs). It realizes the composite-control contract
// for tablists:
//
...
export function useTablist<T extends string>({
  ...
  const tabId = (tab: T): string => `${idBase}-tab-${tab}`
  const panelId = (tab: T): string => `${idBase}-panel-${tab}`
