...
import { configureWindowPlacement, initializeWindowPlacement } from "../../src/main/window-placement";
import { createWindowsPlacement, type WindowsPlacement } from "../../src/main/windows-placement";
import type { WindowPlacementRecord } from "../../src/shared/ui-state";

vi.mock("../../src/main/windows-placement", () => ({ createWindowsPlacement: vi.fn() }));
...
describe("native placement capture", () => {
  ...
  it.each(["native", "logical"] as const)("persists maximized intent when the %s geometry query fails", async (source) => {
    const win = new Window();
    const windows = native();
