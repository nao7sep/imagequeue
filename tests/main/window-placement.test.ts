...
import { applyRestoredBounds, configureWindowPlacement, fitWindowBounds, resolveWindowRestoration } from "../../src/main/window-placement";
...
function setup(
  ...
  write?: (record: WindowPlacementRecord) => void,
) {
  const win = new FakeWindow();
  const saved: WindowPlacementRecord[] = [];
  const onError = vi.fn();
  const persist = write ?? ((record: WindowPlacementRecord) => { saved.push(record); });
...
describe("window placement capture", () => {
  ...
  it("contains a persistence failure", async () => {
    ...
    const { controller, onError } = setup("normal", () => { throw failure; });
    controller.start();
    await controller.flush();
    expect(onError).toHaveBeenCalledWith(failure);
  });
