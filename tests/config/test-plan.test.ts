...
describe('the default test plan', () => {
  ...
  it('typechecks and runs related and repository-reading tests for a TypeScript change', () => {
    expect(plan(['src/main/brainstorm.ts'])).toEqual({
      typecheck: true,
      vitest: ['src/main/brainstorm.ts', ...repositoryReaders],
      live: false,
    })
  })
  ...
  it('typechecks a live test but never runs the live lane', () => {
    expect(plan(['tests/live/main/backends/processor.test.ts'])).toMatchObject({ typecheck: true, live: false })
  })
