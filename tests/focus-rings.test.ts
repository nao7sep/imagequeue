...
// and since Safari 27 so does focus a script moves after keyboard use. The
// Shortcuts, Confirm and Notice bodies and the CLI job log fell back to the
// browser's own ring that way.
//
...
// may ring; a container the covering rule in styles.css reaches; or a site listed
...
const EXPLAINED: Record<string, { kind: Site["kind"]; reason: string }> = {
  "src/renderer/src/components/ElaboratedPromptsModal.tsx": {
    kind: "attribute",
    reason: "a querySelector that finds the tabbable option, not an element",
  },
};
