...
- Each session folder also holds a `session.log`: a per-launch [JSON Lines](https://jsonlines.org) record (one JSON event per line) of what the app did — startup and effective settings, each queued task, a single summary per queue drain (how many generated images succeeded vs. failed, and how long it took), external calls, and every warning or error with its full stack. Per-image generation start/complete are `debug`-level rather than info, so a busy queue summarizes instead of writing two lines per image. `debug`-level lines are diagnostic-only: on in a development build (`npm run dev`) or with `IMAGEQUEUE_DEBUG=1`, off in packaged builds. Logs follow the session folder lifecycle: auto-dropping or deleting a session removes or trashes its `session.log` with it. Secret-bearing fields (API keys) are redacted; attach the relevant `session.log` when filing an issue.
