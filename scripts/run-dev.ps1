...
$rendererUrl = "http://127.0.0.1:20641"
...
try {
    ...
    Write-Step "Replacing any existing ImageQueue runtime"
    Invoke-Native -FilePath "node" -ArgumentList @($runtimeHelper, "claim", $runtimeToken)
    Invoke-Native -FilePath "node" -ArgumentList @($runtimeHelper, "stop", "electron", "ImageQueue", "ImageQueue")
    Invoke-Native -FilePath "node" -ArgumentList @($runtimeHelper, "check-endpoint", "127.0.0.1", "20641")
    ...
    Write-Step "Starting ImageQueue in development mode"
    ...
    Write-Step "ImageQueue is ready at $rendererUrl"
    $devProcess.WaitForExit()
    if ($devProcess.ExitCode -notin @(0, 130, -1073741510)) {
        throw "ImageQueue development runtime failed with exit code $($devProcess.ExitCode)."
    }
}
catch {
    Write-Host ""
    Write-Host "imagequeue run-dev failed: $($_.Exception.Message)" -ForegroundColor Red
    $scriptExitCode = 1
}
finally {
    & node $runtimeHelper is-owner $runtimeToken *> $null
    if ($LASTEXITCODE -eq 0) {
        & node $runtimeHelper stop-if-owner $runtimeToken electron "ImageQueue" "ImageQueue" *> $null
